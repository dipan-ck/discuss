...
// --------------------------------------------------
export async function createSendTransport(channelId: string, userId: string) {
    if (!router) throw new Error("Router not ready");
    ...
    for (const chId in channelTransports) {
        ...
        if (oldTransport) {
            try {
                ...
                console.log(
                    `🧹 Closed old send transport for user ${userId} in channel ${chId}`,
                );
            } catch (e) {
...
// --------------------------------------------------
export async function createRecvTransport(channelId: string, userId: string) {
    if (!router) throw new Error("Router not ready");
    ...
    for (const chId in channelTransports) {
        ...
        if (oldTransport) {
            try {
                ...
                console.log(
                    `🧹 Closed old recv transport for user ${userId} in channel ${chId}`,
                );
            } catch (e) {
    ...
    if (!announcedIp)
        throw new Error("ANNOUNCED_IP missing! Must be public IP.");
...
export async function connectTransport(
    ...
    for (const channelId in channelTransports) {
        ...
        if (transport) {
            ...
            if (state === "connected") {
                console.log(
                    `✅ Transport ${type} for user ${userId} already connected`,
                );
                return true;
            }
            ...
            if (state === "connecting") {
                throw new Error(
                    `Transport ${type} for user ${userId} is already connecting`,
                );
            }
    ...
    for (const chId in channelProducers) {
        ...
        if (oldProducer) {
            try {
                ...
                console.log(
                    `🧹 Closed old producer for user ${userId} in channel ${chId}`,
                );
            } catch (e) {
...
export async function cleanupUserResources(channelId: string, userId: string) {
    console.log(
        `🧹 Cleaning up resources for user ${userId} in channel ${channelId}`,
    );
    ...
    if (channelProducers[channelId]?.[userId]) {
        try {
            ...
            console.log(
                `✅ Closed producer for user ${userId} in channel ${channelId}`,
            );
        } catch (e) {
    ...
    if (channelTransports[channelId]?.[userId]) {
        ...
        if (transports.send) {
            try {
                ...
                console.log(
                    `✅ Closed send transport for user ${userId} in channel ${channelId}`,
                );
            } catch (e) {
        ...
        if (transports.recv) {
            try {
                ...
                console.log(
                    `✅ Closed recv transport for user ${userId} in channel ${channelId}`,
                );
            } catch (e) {
    ...
    if (
        channelTransports[channelId] &&
        Object.keys(channelTransports[channelId]).length === 0
    ) {
        delete channelTransports[channelId];
    }

    if (
        channelProducers[channelId] &&
        Object.keys(channelProducers[channelId]).length === 0
    ) {
